...
product targets two surfaces — a focused **iPhone** prototype for in-the-field
work, and a **desktop** workspace for planning, pipeline review, and offer
ops. Both surfaces share one design language; this document is the source of
truth for tokens, primitives, and the patterns that make the product feel
...
(success-tone arrow + change), and a 110 × 28 sparkline. Stats live in a
4-up grid on desktop, scroll-snap row of 2 on mobile.
...
A small inline-SVG lucide-flavored set lives on `window.Icon` and is
imported wherever needed. Stroke 1.6, rounded caps and joins, 24 × 24
viewBox. The set is closed — adding a new icon means adding a new path
here, not pulling in a library.

Available names: `home, users, user, briefcase, contact, calendar,
check, map, list, board, plus, search, bell, phone, message, note, pen,
share, chevron, down, more, car, clock, flame, arrowUp, arrowRight,
filter, sliders, camera, doc, coins, convert`.

Rules:
- Icons never carry brand color on rest; they sit at `--ink-3` and shift
  to `--brand` only when their parent is selected/active.
- Never use emoji.
...
- **Tap squish** on iOS controls (`transform: scale(0.96)` on `:active`).
- **Now pulse** on the live ribbon dot: 1.6 s ease-in-out, opacity-only.
- **Swipe reveal**: 180 ms ease-out translateX on the inner row.
- **Tweaks panel**: 180 ms slide-up + fade.
...
```
RC CRM Desktop.html     · Desktop entry — loads mobile/data.jsx then desktop/*
RC CRM Mobile.html      · Mobile entry — iPhone bezel + screen switcher
RC CRM Mobile-print.html· Print sheet of every mobile screen at once

desktop/
  styles.css            · Desktop tokens & primitives (this doc § 2–5)
  shell.jsx             · Sidebar + topbar + Money/Countdown/Avatar/Spark
  dashboard.jsx         · Dashboard page composition
  leads.jsx             · Leads list + board
  screens.jsx           · Remaining desktop screens
  app.jsx               · Root router

mobile/
  styles.css            · Mobile tokens (shared scale where possible)
  data.jsx              · MOCK record set + Icon registry (window.MOCK / window.Icon)
  app.jsx               · Root App + screen switcher + tweaks panel
  screen-*.jsx          · One file per screen
  print-app.jsx         · Print-sheet variant
```

Token edits live in **one** place per surface (`desktop/styles.css`,
`mobile/styles.css`); both files import the same conceptual palette and
should be edited in lockstep when the brand shifts.
